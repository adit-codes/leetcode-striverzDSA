...
# ==========================================
    ...
    # --- Post-Login Section ---
        ...
        st.subheader(f"Welcome back, {details['Patient Name']}")
        
        # Step 1: Fetch Parameters if they don't exist
        if st.session_state.dynamic_fields is None:
            if st.button("Fetch My Daily Parameters"):
                with st.spinner("Talking to Workflow Z..."):
                    try:
                        response = requests.post(N8N_WEBHOOK_WORKFLOW_Z, json=details)
                        if response.status_code == 200:
                            # IMPORTANT: Check if response is actually a list
                            data = response.json()
                            st.session_state.dynamic_fields = data
                            st.rerun()
                        else:
                            st.error("Workflow Z couldn't find your profile.")
                    except Exception as e:
                        st.error(f"Error fetching parameters: {e}")
        
        # Step 2: Show Form if parameters are loaded
        else:
            st.write("Please fill in your daily recovery data:")
            with st.form("daily_submission_form"):
                user_responses = {}
                
                # Iterate through parameters fetched from n8n
                for item in st.session_state.dynamic_fields:
                    label = item.get("parameter")
                    dtype = item.get("datatype", "text").lower()
                    
                    if dtype == "number":
                        user_responses[label] = st.number_input(f"Enter {label}", key=f"inp_{label}")
                    elif dtype == "text":
                        user_responses[label] = st.text_area(f"Enter {label}", key=f"inp_{label}")
                    elif dtype in ["video", "audio", "image"]:
                        # Since you mentioned issues with direct input, 
                        # using file_uploader is more reliable than just a URL text box
                        user_responses[label] = st.file_uploader(f"Upload {label} ({dtype})", key=f"inp_{label}")
